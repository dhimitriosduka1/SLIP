...
def get_args_parser():
    parser = argparse.ArgumentParser(
        description="Linear probe evaluation", add_help=False
    )
    ...
    parser.add_argument(
        "--pretrained", default="", type=str, help="path to CLIP pretrained checkpoint"
    )
    return parser
...
def main(args):
    ...
    if os.path.isfile(args.pretrained):
        ...
        raise Exception(
            "Missing pretrained model checkpoint: {}".format(args.pretrained)
        )
    ...
    args.workers = int(
        (args.workers + utils.get_world_size() - 1) / utils.get_world_size()
    )
    ...
    train_dataset = datasets.get_downstream_dataset(
        catalog, args.dataset, is_train=True, transform=train_transform
    )
    val_dataset = datasets.get_downstream_dataset(
        catalog, args.dataset, is_train=False, transform=val_transform
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if utils.is_main_process():  # only the first GPU saves checkpoint
            ...
            if epoch == args.start_epoch:
                sanity_check(
                    model.state_dict(), args.pretrained, linear_keyword, visual_keyword
                )
        ...
        if utils.is_main_process():
            with open(
                os.path.join(
                    args.output_dir,
                    "linear_{}_lr={}_log.txt".format(args.dataset, args.lr),
                ),
                "a",
            ) as f:
...
def sanity_check(state_dict, pretrained_weights, linear_keyword, visual_keyword):
    ...
    for k in list(state_dict.keys()):
        ...
        k_pre = (
            visual_keyword + k[len("module.") :]
            if k.startswith("module.")
            else visual_keyword + k
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Linear probe evaluation", parents=[get_args_parser()]
    )
